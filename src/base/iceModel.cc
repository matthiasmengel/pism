...
PetscErrorCode IceModel::createVecs() {
  ...
  if (config.get_flag("part_grid") == true) {
    ...
    ierr = vPgdiag.create(grid, "Pgdiag", true); CHKERRQ(ierr);
    ierr = vPgdiag.set_attrs("diagnostic", "diagnostic pg mask, 1=floating, 2=grounded", "", ""); CHKERRQ(ierr);
    ierr = variables.add(vPgdiag); CHKERRQ(ierr);
